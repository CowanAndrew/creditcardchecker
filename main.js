...
function validateCred(cardDigits) {
  const digits = cardDigits.slice().reverse(); // Create a copy of the array and reverse it
    let sum = 0;

    for (let i = 0; i < digits.length; i++) {
        let digit = digits[i];

        // Double every other digit starting from the second to the right
        if (i % 2 === 1) {
            digit *= 2;
...
const cardNumber = [4, 5, 3, 2, 7, 7, 8, 7, 7, 1, 0, 9, 1, 7, 9, 5];
console.log(validateCred(cardNumber)); // Output will be true or false depending on the validity of the card number
...
console.log(idInvalidCardCompanies(invalidNumbers)); // Output: [ 'Amex', 'Mastercard', 'Visa', 'Discover' ]
